"""Network probes for dashboard.py without Flask dependencies."""
...
from sys_probe import port_open as _sys_port_open, run as _sys_run
...
def run(cmd_list, timeout):
    ...
    if dashboard is not None:
        current = getattr(dashboard, "run", _sys_run)
        if current is not run:
            return current(cmd_list, timeout)
    return _sys_run(cmd_list, timeout)


def port_open(host, port, timeout=0.5):
    dashboard = sys.modules.get("dashboard")
    if dashboard is not None:
        current = getattr(dashboard, "port_open", _sys_port_open)
        if current is not port_open:
            return current(host, port, timeout=timeout)
    return _sys_port_open(host, port, timeout=timeout)
