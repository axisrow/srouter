...
# Tunnel quality trend (observe-only): the watchdog already curls the tunnel every ~20s;
# it records that same request's timing split (connect / path+TLS / server response /
# total) to ~/Library/Logs/srouter-watchdog.metrics.jsonl once a minute — ZERO extra network
# traffic. The dashboard "Tunnel quality (trend)" panel compares the 15-min median against a
# "same hour, past days" baseline and shows degradation without acting on it (automation
# ladder: observe first). Config (srouter.local.json -> probes): metrics_enabled (default
# true), metrics_interval_sec (60), metrics_retention_days (7).
# The passive 5xx-rate row needs the privoxy access log (debug 1, see the note above);
# without it the row just shows "disabled".
