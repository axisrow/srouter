...
def _codex_app_proxy_check():
    ...
    if not keys:
        if not rust_pids:
            ...
            # down здесь клепал пары пушей «+»/«−» (класс «драйвер по природе флапует»).
            # observe-статус (unknown → info): реальный утечный маршрут ловит
            # _codex_app_chromium_proxy_check и runtime-ветки ниже.
