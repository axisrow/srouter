...
def test_probe_hot_routes_clamps_subsecond_update_interval(monkeypatch, tmp_path):
    state_path = tmp_path / "srouter.local.json"
    cache_path = tmp_path / "srouter.hot_routes.json"
    log_path = tmp_path / "privoxy.log"
    _write_state(state_path, {"enabled": True, "update_interval_sec": 0.05})

    opts = dashboard_hotroutes._options(
        {"hot_routes": {"enabled": True, "update_interval_sec": 0.05}}
    )
    assert opts["update_interval"] >= 1.0
