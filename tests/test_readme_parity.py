...
# Маркеры «это ещё не готово / это ручное действие по рекомендации» — блок с любым из них
# НЕ считается обещанием автоматики. Покрывает и roadmap-пометки, и честные переформулировки
# («ранжирует/рекомендует лучший, переключение вручную»), согласованные со стр. про manual switch.
_ROADMAP_MARKERS = (
    # roadmap / «этого ещё нет»
    "в планах",
    "roadmap",
    "no auto‑failover",
    "no auto-failover",
    ...
    # honest-маркеры: действие ручное / это лишь рекомендация, а не автоматика.
    # ВАЖНО: только маркеры, отменяющие автоматику (ручное действие / совет). «ранжирует/ranked»
    # НЕ входят — ранжирование само по себе не означает ручное переключение (node_selector тоже
    # ранжирует), и как honest-маркер оно ложно прикрывало бы overclaim «traffic goes to the best»
    # в том же буллете (cycle-2 ревью).
    "вручную",
    "рекоменд",   # рекомендует / рекомендация / по рекомендации
    "manual",
    "recommend",  # recommend / recommendation / recommends
)

# Фразы-обещания node auto-failover / авто-выбора узла как готовой фичи.
_AUTOFAILOVER_CLAIMS = (
    ...
    # кластер «авто-выбор быстрейшего узла» (ревью PR #87): node_selector даёт совет + ручной
    # select_node, а не автоматический выбор. Ловим утвердительные обещания автоматического выбора.
    "сам выберет",
    "сам выбирает",
    "автоматического выбора",
    "автоматический выбор",
    "автоматически восстанавливает пропущенные узлы",
    "picks the fastest",
    "fastest connected",
    # cycle-2 ревью: граница привязана к СЕМАНТИКЕ, а не к локали — те же обещания по-английски
    # и остаточная «балансировка/трафик идёт на лучший узел» (тоже подразумевает авто-переключение).
    "restores network nodes",
    "traffic goes to the best",
    "трафик балансируется на узел",
    "трафик идёт на узел",
    # cycle-3 ревью: routing-shorthand в ASCII-схемах цепочки («… → лучший узел» / «→ best node»)
    # тоже подразумевает авто-выбор. Ловим форму со стрелкой, чтобы не задеть честное
    # «рекомендует лучший узел (вручную)» — трафик идёт на ВЫБРАННЫЙ активный узел, не на «лучший».
    "→ лучший узел",
    "→ best node",
)
...
def _offending_lines(claims):
    ...
    """
    offending = []
    # Гранулярность = один markdown-буллет / строка таблицы / абзац. Разбиваем по пустой строке,
    # по началу буллета ('\n- ') И по началу табличной строки ('\n|'): иначе список
    # Self-optimization или таблица слипаются в один блок, и roadmap/honest-маркер из одной
    # строки замаскировал бы overclaim в соседней строке того же блока (ложноотрицательный тест).
    blocks = re.split(r"\n\s*\n|\n(?=- )|\n(?=\|)", README)
    for block in blocks:
